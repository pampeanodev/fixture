...
export function SimulatorView() {
  ...
  function handleSimulate() {
    ...
    const rankingBefore = computeRanking(state, t("common.youFallback"));
  ...
  function getWinnerText(
    ...
    if (score.home > score.away) {
      return t("simulator.winner", { team: homeName ?? t("simulator.winnerHomeFallback") });
    }
    if (score.away > score.home) {
      return t("simulator.winner", { team: awayName ?? t("simulator.winnerAwayFallback") });
    }
    if (score.penalties) {
      const winner = score.penalties.home > score.penalties.away ? homeName : awayName;
      return t("simulator.penaltiesWinner", {
        home: score.home,
        away: score.away,
        team: winner ?? "",
      });
    }
    return t("simulator.draw");
  }

  if (lastResult) {
    const rankingAfter = computeRanking(state, t("common.youFallback"));
    const prevMatch =
      lastResult.kind === "group"
        ? state.groupMatches.find((m) => m.id === lastResult.matchId)
        : resolvedKnockout.find((m) => m.id === lastResult.matchId);

    const prevHomeTeam = state.teams.find((team) => team.id === prevMatch?.homeTeamId);
    const prevAwayTeam = state.teams.find((team) => team.id === prevMatch?.awayTeamId);

    const prevHomeName = prevHomeTeam ? t(`teams.${prevHomeTeam.id}`) : undefined;
    const prevAwayName = prevAwayTeam ? t(`teams.${prevAwayTeam.id}`) : undefined;

    const winnerText = getWinnerText(lastResult.score, prevHomeName, prevAwayName);
      ...
      <div className="simulator-view">
        <div className="simulator-header">
          ...
          <div className="simulator-header-actions">
            <button className="sim-btn" onClick={handleReset}>{t("simulator.reset")}</button>
            <button className="sim-btn danger" onClick={handleExit}>{t("simulator.exit")}</button>
          </div>
        </div>
        ...
        <div className="simulator-match">
          <div className="simulator-match-teams">
            <div className="sim-team">
              ...
              <span className="sim-team-name">{prevHomeName ?? ""}</span>
            </div>
            <span className="sim-final-score">
              ...
              {lastResult.score.penalties && (
                <span className="sim-pen">
                  {" "}({t("simulator.penShort")} {lastResult.score.penalties.home}-{lastResult.score.penalties.away})
                </span>
              )}
            </span>
            <div className="sim-team">
              <span className="sim-team-flag">{prevAwayTeam?.flag}</span>
              <span className="sim-team-name">{prevAwayName ?? ""}</span>
            </div>
          </div>
        ...
        <div className="simulator-deltas">
          <h3>{t("simulator.pointsTitle")}</h3>
          <table className="simulator-predictions-table">
            <tbody>
              {deltas.map((d) => (
                <tr key={d.player.name} className={d.player.isLocal ? "local" : ""}>
                  <td>{d.player.name}{d.player.isLocal && <span className="local-tag"> {t("simulator.youSuffix")}</span>}</td>
        ...
        <div className="simulator-deltas">
          <h3>{t("simulator.currentRankingTitle")}</h3>
          <table className="simulator-predictions-table">
            <tbody>
              {deltas.map((d) => {
                ...
                return (
                  <tr key={d.player.name} className={d.player.isLocal ? "local" : ""}>
                    <td>{d.posAfter + 1}.</td>
                    <td>{d.player.name}{d.player.isLocal && <span className="local-tag"> {t("simulator.youSuffix")}</span>}</td>
                    <td className="pred-score">{d.player.total} {t("simulator.pointsSuffix")}</td>
    ...
    return (
      <div className="simulator-view">
        <div className="simulator-empty">
          <h2>{t("simulator.titleComplete")}</h2>
          <p>{t("simulator.noMoreMatches")}</p>
          <div className="simulator-actions">
            <button className="sim-btn primary" onClick={handleExit}>
              {t("simulator.exitAndReturn")}
            </button>
            <button className="sim-btn" onClick={handleReset}>
              {t("simulator.simulateAgain")}
            </button>
          </div>
        </div>
      </div>
  ...
  const stageLabel =
    pending.kind === "group"
      ? t("simulator.stageLabel.group", { group: (match as { group?: string }).group ?? "" })
      : t(`simulator.stageLabel.${(match as { round: KnockoutRound }).round}`);

  const matchId = match.id;
  const localName = state.playerName.trim() || t("common.youFallback");
    ...
    <div className="simulator-view">
      ...
      <div className="simulator-match">
        <div className="simulator-match-meta">
          {stageLabel} · {formatDate(match.dateUtc)}
        </div>
        <div className="simulator-match-teams">
          <div className="sim-team">
            <span className="sim-team-flag">{homeTeam?.flag}</span>
            <span className="sim-team-name">{homeTeam ? t(`teams.${homeTeam.id}`) : ""}</span>
          </div>
          <span className="sim-vs">{t("simulator.vs")}</span>
          <div className="sim-team">
            <span className="sim-team-flag">{awayTeam?.flag}</span>
            <span className="sim-team-name">{awayTeam ? t(`teams.${awayTeam.id}`) : ""}</span>
          </div>
        </div>
      </div>
      ...
      {manualEntry ? (
        ...
        <>
          <div className="simulator-predictions">
            <h3>{t("simulator.predictionsTitle")}</h3>
            {predictionRows.length === 0 ? (
              <p className="simulator-predictions-empty">{t("simulator.predictionsEmpty")}</p>
            ) : (
              <table className="simulator-predictions-table">
                <tbody>
                  {predictionRows.map((row) => (
                    <tr key={row.name} className={row.isLocal ? "local" : ""}>
                      <td>{row.name}{row.isLocal && <span className="local-tag"> {t("simulator.youSuffix")}</span>}</td>
                      <td className="pred-score">
                        {row.score ? `${row.score.home} - ${row.score.away}` : t("simulator.noScore")}
                      </td>
                    </tr>
                  ))}
                </tbody>
